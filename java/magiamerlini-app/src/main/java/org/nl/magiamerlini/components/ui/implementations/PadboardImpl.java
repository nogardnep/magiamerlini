package org.nl.magiamerlini.components.ui.implementations;

import java.util.ArrayList;
import java.util.List;

import org.nl.magiamerlini.Configuration;
import org.nl.magiamerlini.communication.api.Communication;
import org.nl.magiamerlini.communication.tools.CommunicatingComponent;
import org.nl.magiamerlini.components.ui.api.Padboard;
import org.nl.magiamerlini.components.ui.tools.Color;
import org.nl.magiamerlini.components.ui.tools.Pad;
import org.nl.magiamerlini.controllers.api.MainController;
import org.nl.magiamerlini.data.tools.Item;

public class PadboardImpl extends CommunicatingComponent implements Padboard {
	private final static String COMPONENT_NAME = "pad";
	
	private List<Pad> pads;

	public PadboardImpl(Communication communication) {
		super(communication);
		pads = new ArrayList<Pad>();

		for (int i = 0; i < Configuration.Pads.getValue(); i++) {
			pads.add(new Pad(i));
		}
	}

	@Override
	public void update() {
		for (Pad pad : pads ) {
			updatePad(pad);
		}
	}

	@Override
	public void updatePad(int padNum) {
		updatePad(pads.get(padNum));
	}
	
	private void updatePad(Pad pad) {
		Item item = mainController.getItemCorrespondingToPad(pad.getNumber());

		pad.setIntensity(1);
		pad.setSpeed(Pad.SPEED_FOR_NO_BLINKING);

		if (mainController.getItemsSelector().isFirstSelected(item)) {
			pad.setColor(Color.Orange);
		} else if (mainController.getItemsSelector().isSelected(item)) {
			pad.setColor(Color.Yellow);
		} else {
			pad.setColor(Color.Green);
			//pad.setIntensity(0);
		}

		displayPad(pad);
		
	}

	private void displayPad(Pad pad) {
		communication.sendMessage(COMPONENT_NAME + " " + pad.getNumber() + " intensity " + pad.getIntensity());
		communication.sendMessage(COMPONENT_NAME + " " + pad.getNumber() + " color " + String.valueOf(pad.getColor()).toLowerCase());
		communication.sendMessage(COMPONENT_NAME + " " + pad.getNumber() + " speed " + pad.getSpeed());
	}
}
